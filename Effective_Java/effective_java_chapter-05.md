...
이런 모든 쓰임에서 매개변수화되는 대상은 원소가 아닌 컨테이너 자신임.

따라서 하나의 컨테이너에서 매개변수화 할 수 있는 타입의 수가 제한됨.

Set은 원소의 타입을 뜻하는 단 하나의 타입 매개변수만 있으면 되고, Map에는 키와 값의 타입을 뜻하는 2개만 필요한 식임.

---

하지만 더 유연한 수단이 필요할 때도 종종 있음.

데이터베이스의 행은 임의 개수의 열을 가질 수 있는데, 열들을 모두 타입 안전하게 이용할 수 있다면 좋을것.
